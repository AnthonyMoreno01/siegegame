...
void draw_playfield() {
  	...
  	cputcxy(12,1,players[0].score+'0');
 	cputsxy(5,1,"Round:");
  	//cputcxy(23,1,players[0].round_score+'0');
      	cputsxy(17,1,"Tokens:");
}
...
void game_over(byte gameover) {
  ...
  cputsxy(11,14,"Rounds:");
  cputcxy(19,14, '0'+gameover);
  vrambuf_flush();
  delay(75);
  gameover = 1;
  players[0].score = 0;
...
void play() {
  ...
  spawn_item();
  cputsxy(17,1,"Tokens:");
  cputcxy(25,1,players[0].round_score+'0');
  ...
  while (1) {
    ...
    if (players[0].token_collected){
     players[0].round_score++; 
      vrambuf_flush();
      cputcxy(25,1,players[0].round_score+'0');
      players[0].token_collected = 0;
    }
    
    if(players[0].round_score == players[0].score+1){
      	players[0].score++;
      	frames_per_move--;
      	ppu_off();
  	setup_attrib_table();
